...
if(isset($_POST["supplier_edit"])){
	...
	if($err==""){
		$sql="Update supplier set `supplier_name`='".slash($supplier_name)."',`phone`='".slash($phone)."', `address`='".slash($address)."', `balance`='".slash($balance)."', `supplier_code`='".slash($supplier_code)."'"." where id='".$id."'";
		doquery($sql,$dblink);
