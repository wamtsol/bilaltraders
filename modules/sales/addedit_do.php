...
if(isset($_POST["action"])){
	...
	switch($_POST["action"]){
		...
		case "get_purchase_items":
			...
			$rs = doquery( "SELECT a.*, c.supplier_code, c.supplier_name, d.title as size_title, e.title as color_title FROM `purchase_items` a left join purchase b on a.purchase_id = b.id left join supplier c on b.supplier_id = c.id left join size d on a.size = d.id left join color e on a.color = e.id where b.status=1 order by supplier_code, item_number", $dblink );
			$purchase_items = array();
			if( numrows( $rs ) > 0 ) {
				while( $r = dofetch( $rs ) ) {
					$item_name = (!empty($r[ "supplier_code" ])?unslash($r[ "supplier_code" ])."-":'').unslash($r[ "item_number" ]);
			...
			if( count( $err ) == 0 ) {
				...
				foreach( $sales->items as $item ) {
					if( isset( $item->sales_itemid ) && !isset( $item->sales_itemid ) ) {
						...
						doquery( "update sales_items set `purchase_item_id`='".slash( $item->purchase_item_id )."', `sale_price`='".$item->sale_price."', `discount`='".$item->discount."', `quantity`='".$item->quantity."', `total`='".$item->total."' where id='".$item->sales_itemid."'", $dblink );
						...
						doquery( "insert into sales_items ( sales_id, purchase_item_id, sale_price, discount, quantity, total ) values( '".$sales_id."', '".$item->purchase_item_id."', '".$item->sale_price."', '".$item->discount."', '".$item->quantity."', '".$item->total."' )", $dblink );
