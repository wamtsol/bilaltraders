...
if(isset($_POST["items_add"])){
	...
	if($err==""){
		$sql="INSERT INTO items (item_category_id, title, unit_id, alert_quantity, unit_price, quantity, sortorder) VALUES ('".slash($item_category_id)."','".slash($title)."','".slash($unit_id)."','".slash($alert_quantity)."','".slash($unit_price)."','".slash($quantity)."','".slash($sortorder)."')";
		doquery($sql,$dblink);
		$id = inserted_id();
