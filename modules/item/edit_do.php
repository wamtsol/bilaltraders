...
if(isset($_POST["items_edit"])){
	...
	if($err==""){
		$sql="Update items set `item_category_id`='".slash($item_category_id)."', `title`='".slash($title)."', `unit`='".slash($unit)."',  `unit_price`='".slash($unit_price)."',`quantity`='".slash($quantity)."',`sortorder`='".slash($sortorder)."' where id='".$id."'";
		doquery($sql,$dblink);
