...
if(isset($_GET["account_type"])){
	$account_type=slash($_GET["account_type"]);
	$_SESSION["account"]["list"]["account_type"]=$account_type;
}
if(isset($_SESSION["account"]["list"]["account_type"]))
	$account_type=$_SESSION["account"]["list"]["account_type"];
else
	$account_type="";
if($account_type!=""){
	$extra.=" and type='".$account_type."'";
	$is_search=true;
}
?>
                ...
                <div class="col-sm-3">
                	<select name="account_type" id="account_type" title="Choose Option">
						...
						foreach ($account_types as $key=>$value) {
                            ?>
                            <option value="<?php echo $key?>"<?php echo ($account_type!="" && $key==$account_type)?' selected="selected"':""?>><?php echo $value ?></option>
                            <?php
                        }
                        ?>
                    </select>
