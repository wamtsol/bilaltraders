...
if(isset($_POST["account_edit"])){
	...
	if($err==""){
		$sql="Update account set `title`='".slash($title)."', `type`='".slash($type)."',`balance`='".slash($balance)."',`is_petty_cash`='".slash($is_petty_cash)."', `description`='".slash($description)."' where id='".$id."'";
		doquery($sql,$dblink);
