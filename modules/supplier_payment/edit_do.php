...
if(isset($_POST["supplier_payment_edit"])){
	...
	if($err==""){
		$sql="Update supplier_payment set `supplier_id`='".slash($supplier_id)."',`datetime_added`='".slash(datetime_dbconvert(unslash($datetime_added)))."', `amount`='".slash($amount)."',`account_id`='".slash($account_id)."',`details`='".slash($details)."'"." where id='".$id."'";
		doquery($sql,$dblink);
