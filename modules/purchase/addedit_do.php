		...
		case "save_purchase":
			...
			if( count( $purchase->items ) == 0 ) {
				...
				foreach( $purchase->items as $item ) {
					if( empty( $item->item_id ) || empty( $item->purchase_price ) || empty( $item->quantity ) ){
			...
			if( count( $err ) == 0 ) {
				...
				foreach( $purchase->items as $item ) {
					if( empty( $item->id ) ) {
						doquery( "insert into purchase_items( purchase_id, item_id, purchase_price, quantity, total, is_return ) values( '".$purchase_id."', '".slash( $item->item_id )."',  '".$item->purchase_price."', '".$item->quantity."', '".$item->total."', '".$item->return."' )", $dblink );
