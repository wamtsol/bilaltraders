...
if(isset($_POST["unit_edit"])){
	...
	if($err==""){
		$sql="Update units set `title`='".slash($title)."', `short_title`='".slash($short_title)."' where id='".$id."' ";
		doquery($sql,$dblink);
		unset($_SESSION["unit_manage"]["edit"]);
		header('Location: unit_manage.php?tab=list&msg='.url_encode("Successfully Updated"));
		die;
	}
	else{
...
if(isset($_GET["id"]) && $_GET["id"]!=""){
	$rs=doquery("select * from units where id='".slash($_GET["id"])."'",$dblink);
	if(numrows($rs)>0){
		$r=dofetch($rs);
