...
$account_types = array("Current Assets","Fixed Assets","Capital");
function admin_logo(){
	global $admin_logo, $site_title, $site_url;
	echo '<a href="'.$site_url.'/sweetshop">';
	if(!empty($admin_logo)){
...
$product_type = array("Simple Product","Variable Product");
...
function get_order( $id ) {
	...
	$order = array(
		"id" => $id,
		"datetime_added" => date("h:i A", strtotime($order[ "datetime_added" ])),
	...
	if( numrows($rs) > 0 ) {
		while( $r = dofetch( $rs ) ) {
			$item = dofetch( doquery( "SELECT a.*, c.supplier_code, c.supplier_name, d.title as size_title, e.title as color_title FROM `purchase_items` a left join purchase b on a.purchase_id = b.id left join supplier c on b.supplier_id = c.id left join size d on a.size = d.id left join color e on a.color = e.id where a.id = '".$r[ "purchase_item_id" ]."'", $dblink ) );
			$item_name = (!empty($item[ "supplier_code" ])?unslash($item[ "supplier_code" ])."-":'').unslash($item[ "item_number" ]);
			if( !empty( $item[ "size" ] ) || !empty( $item[ "color" ] ) ) {
				$attributes = array();
				if( !empty( $item[ "size" ] ) ) {
					$attributes[] = 'Size: '.unslash( $item[ "size_title" ] );
				}
				if( !empty( $item[ "color" ] ) ) {
					$attributes[] = 'Color: '.unslash( $item[ "color_title" ] );
				}
...
function get_order_return( $id ) {
	global $dblink;
	$order_return= dofetch(doquery("select * from sales_return where id='".$id."'", $dblink));
	$order_return = array(
		"id" => $id,
		"datetime_added" => date("h:i A", strtotime($order_return[ "datetime_added" ])),
		"discount" => $order_return[ "discount" ],
		"total" => $order_return[ "net_price" ],
		"items" => array()
	);
...
function get_exchange_order( $id ) {
	global $dblink;
	$exchange_order= dofetch(doquery("select * from sales where id='".$id."'", $dblink));
...
function update_stock( $item_id ){
	global $dblink;
	doquery("update `items` set quantity = (select sum(quantity) from items_variations where item_id = '".$item_id."') where id='".$item_id."'", $dblink);
}
...
function get_supplier_balance( $supplier_id, $dt = 0 ){
	...
	if( numrows( $supplier ) > 0 ) {
		...
		$sql="select sum(amount) as amount from (select concat( 'Purchase #', id) as transaction, total_price as amount from purchase where supplier_id = '".$supplier_id."' and datetime_added <='".$dt."' union select concat( 'Purchase Return #', id) as transaction, -net_price as amount from purchase_return where supplier_id = '".$supplier_id."' and datetime_added <='".$dt."' union select concat( 'Payment #', id) as transaction, -amount from supplier_payment where supplier_id = '".$supplier_id."' and datetime_added <='".$dt."') as transactions";
		$balance=dofetch(doquery($sql,$dblink));
